...
def download_pdf_from_url(url, output_path, max_pages=2):
  """Downloads a PDF file from the specified URL and truncates it to max_pages."""
  print(f"Downloading PDF from {url}...")
  response = requests.get(url, stream=True)
  response.raise_for_status()  # Raise an exception for HTTP errors
  
  # Create a temporary file to store the original PDF
  temp_path = output_path + ".full"
  ...
  with open("", 'wb') as f:
      pdf_writer.write(f)
  print(f"Truncated PDF saved to local storage: {""}")

  # Remove the temporary file
  os.remove(temp_path)
  
  print(f"Pages kept: {pages_to_keep}")
  print(f"Pages removed: {original_pages - pages_to_keep}")
  print(f"Reduction: {((original_pages - pages_to_keep)/original_pages)*100:.1f}%")
  print(f"\nTruncated PDF saved to {output_path}")
  return output_path
...
def main():
  ...
  with tempfile.TemporaryDirectory() as temp_dir:
    ...
    print(f"len of data {len(pdf_path)}")
